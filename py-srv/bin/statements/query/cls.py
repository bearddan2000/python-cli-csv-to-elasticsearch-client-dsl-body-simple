...
class ClientQuery(Statements):
    ...
    def refresh_body(self, query: dict, func_name: str):
        self.es.indices.refresh(index=INDEX_NAME)
        collection=self.es.search(index=INDEX_NAME, body=query)
        print_result(collection['hits']['hits'], self.server, self.__class__, func_name+'_sql')
    ...
    def range_query(self):
        self.refresh_body({'query': RANGE}, currentFuncName())
        self.refresh_query(RANGE, currentFuncName())

    def regex_query(self):
        self.refresh_body({'query': REGEX}, currentFuncName())
        self.refresh_query(REGEX, currentFuncName())

    def must_filter_query(self):
        self.refresh_body({'query': MUST_FILTER}, currentFuncName())
        self.refresh_query(MUST_FILTER, currentFuncName())

    def must_not_query(self):
        self.refresh_body({'query': MUST_NOT_FILTER}, currentFuncName())
        self.refresh_query(MUST_NOT_FILTER, currentFuncName())

    def must_query(self):
        self.refresh_body({'query': MUST}, currentFuncName())
        self.refresh_query(MUST, currentFuncName())

    def filter_query(self):
        self.refresh_body({'query': FILTER}, currentFuncName())
        self.refresh_query(FILTER, currentFuncName())

    def get_all_query(self):
        self.refresh_body({'query': SELECT_ALL}, currentFuncName())
